...
Ext.define('CRMRE.view.buildings.BuildingPhotos',{
	extend: 'Ext.panel.Panel',
	frame: true,
	id: 'building-images-view',
        ...
        this.items = [
        {
        	...
        	[{
	            ...
	            handler: function(event, toolEl, panel){
	                ...
	                if (Ext.Array.indexOf(CRMRE.global.Vars.user_perms,'add_buildingphotos')!=-1) {
				        if(form.isValid()){
				            form.submit({
				                url: '/building_photo_upload/',
				                ...
				                Ext.Msg.alert('Успешная загрузка!', o.result.message);
				                    Ext.data.StoreManager.lookup('BuildingPhotos').load({params: {building:form.getValues().building}}),
	        	...
	        	type: 'close',
	            handler: function(event, toolEl, panel){
	            	if (Ext.Array.indexOf(CRMRE.global.Vars.user_perms,'delete_buildingphotos')!=-1) {
		                var form = this.up('form').getForm();
		                if (typeof(form.select_records) != "undefined" && form.select_records.length) {
					        var store = Ext.data.StoreManager.lookup('BuildingPhotos');
        	...
        	xtype: 'panel',
        	id: "building_photo_list",
        	frame: true,
        	flex: 1,
        	items: Ext.create('Ext.view.View', {
        		autoHeight: true, 
				autoScroll: true,
	            store: Ext.data.StoreManager.lookup('BuildingPhotos'),
	            tpl: [
	                '<tpl for=".">',
	            ...
	            listeners: {
	                selectionchange: function(dv, nodes ){
	                    var l = nodes.length;
	                    var form = Ext.getCmp('tabpanel').getActiveTab().down('#building_form_photo').getForm().select_records = nodes;
