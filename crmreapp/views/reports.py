...
def reports(request):
    ...
    if request.method == 'POST':
        ...
        if report_type == "kpi":
            ...
            group_type = int(getparams.get('group_type'))
            content_item = {}
            ...
            if group_type == 1:
                group_list = model_object_category.objects.all().order_by("pk")
            else:
                group_list = User.objects.values_list('brigade', flat=True).distinct().order_by("brigade")
            for object_group in group_list:

                if group_type == 1:
                    object_category = [object_group]
                    content_item["category_name"] = object_group.name
                    filter_group = Q(object_category=object_group)
                    filter_brigade = Q()
                    filter_brigade_show = Q()
                    users_list = object_group.group.user_set.filter(is_active=True)
                else:
                    content_item["category_name"] = " - ".join(("Группа",str(object_group)) )
                    object_category = model_object_category.objects.all().order_by("pk")
                    filter_group = Q()
                    filter_brigade = Q(performer__brigade=object_group)
                    filter_brigade_show = Q(offer__order_buy__performer__brigade = object_group)
                    users_list = User.objects.filter(brigade=object_group).filter(is_active=True)

                content_item["category_type"] = 1
                # нормы
                content_item["rate_growth_orders"] = int(round((object_category[0].rate_growth_orders * time_interval)/30.0))
                content_item["rate_growth_contracts"] = int(round((object_category[0].rate_growth_contracts * time_interval)/30.0))
                content_item["rate_shows"] = int(round((object_category[0].rate_shows * time_interval)/30.0))
                content_item["rate_transactions"] = int(round((object_category[0].rate_transactions * time_interval)/30.0))
                # заявки: предложение/спрос
                content_item["orders_all"] = "/".join((str(model_order_sale.filter(filter_group).filter(filter_brigade).filter(create_date__lte=date_stop).filter(hystoryordersalestatus__status__in=status_works).exclude(hystoryordersalestatus__status__in=status_noworks).distinct().count()),str(model_order_buy.filter(filter_brigade).filter(filter_group).filter(create_date__lte=date_stop).filter(hystoryorderbuystatus__status__in=status_works).exclude(hystoryorderbuystatus__status__in=status_noworks).distinct().count())))
                content_item["orders_growth"] = "/".join((str(model_order_sale.filter(filter_group).filter(filter_brigade).filter(create_date__range=(date_start,date_stop)).filter(hystoryordersalestatus__status__in=status_works_all).exclude(hystoryordersalestatus__status__in=status_noworks_all).distinct().count()),str(model_order_buy.filter(filter_brigade).filter(filter_group).filter(create_date__range=(date_start,date_stop)).filter(hystoryorderbuystatus__status__in=status_works_all).exclude(hystoryorderbuystatus__status__in=status_noworks_all).distinct().count())))
                content_item["orders_growth_rn"] = ""
                content_item["orders_growth_agent"] = ""
                content_item["orders_retired"] = "/".join((str(model_order_sale.filter(filter_brigade).filter(filter_group).filter(hystoryordersalestatus__date__range=(date_start,date_stop),hystoryordersalestatus__status=status_retired).distinct().count()),str(model_order_buy.filter(filter_brigade).filter(filter_group).filter(hystoryorderbuystatus__date__range=(date_start,date_stop),hystoryorderbuystatus__status=status_retired).distinct().count())))
                # договоры: предложение/спрос
                content_item["orders_contract_all"] = "/".join((str(model_order_sale.filter(filter_brigade).filter(filter_group).filter(contract_date__lte=date_stop,contract_type__in=(contract_all)).exclude(hystoryordersalestatus__status__in=status_noworks).distinct().count()),str(model_order_buy.filter(filter_brigade).filter(filter_group).filter(contract_date__lte=date_stop,contract_type__in=(contract_all)).exclude(hystoryorderbuystatus__status__in=status_noworks).distinct().count())))
                content_item["orders_contract_growth"] = "/".join((str(model_order_sale.filter(filter_brigade).filter(filter_group).filter(contract_date__range=(date_start,date_stop),contract_type__in=(contract_all)).exclude(hystoryordersalestatus__status__in=status_noworks_all).distinct().count()),str(model_order_buy.filter(filter_brigade).filter(filter_group).filter(contract_date__range=(date_start,date_stop),contract_type__in=(contract_all)).exclude(hystoryorderbuystatus__status__in=status_noworks_all).distinct().count())))
                # актов показов за период
                content_item["shows_acts_all"] = str(models.HystoryShow.objects.filter(filter_brigade_show).filter(offer__order_buy__object_category__in=object_category,date__range=(date_start,date_stop)).exclude(result=status_show_work).distinct().count()*2)
                # сделок за период
                content_item["transactions"] = str(model_order_sale.filter(filter_brigade).filter(filter_group).filter(offer__hystoryservice__date__range=(date_start,date_stop),offer__hystoryservice__operation=operation_registr,offer__hystoryservice__result_operation=operation_complet).distinct().count()+model_order_buy.filter(filter_brigade).filter(filter_group).filter(offer__hystoryservice__date__range=(date_start,date_stop),offer__hystoryservice__operation=operation_registr,offer__hystoryservice__result_operation=operation_complet).distinct().count())
                data["messages"]["content"].append(content_item.copy())
                agent_count = 0
                for user in users_list:
                    ...
                    # заявки: предложение/спрос
                    content_item["orders_all"] = "/".join((str(model_order_sale.filter(filter_group).filter(performer=user,create_date__lte=date_stop).filter(hystoryordersalestatus__status__in=status_works).exclude(hystoryordersalestatus__status__in=status_noworks).distinct().count()),str(model_order_buy.filter(filter_group).filter(performer=user,create_date__lte=date_stop).filter(hystoryorderbuystatus__status__in=status_works).exclude(hystoryorderbuystatus__status__in=status_noworks).distinct().count())))
                    content_item["orders_growth"] = "/".join((str(model_order_sale.filter(filter_group).filter(performer=user,create_date__range=(date_start,date_stop)).filter(hystoryordersalestatus__status__in=status_works_all).exclude(hystoryordersalestatus__status__in=status_noworks_all).distinct().count()),str(model_order_buy.filter(filter_group).filter(performer=user,create_date__range=(date_start,date_stop)).filter(hystoryorderbuystatus__status__in=status_works_all).exclude(hystoryorderbuystatus__status__in=status_noworks_all).distinct().count())))
                    content_item["orders_growth_agent"] = "/".join((str(model_order_sale.filter(filter_group).filter(author=user,create_date__range=(date_start,date_stop)).filter(hystoryordersalestatus__status__in=status_works_all).exclude(hystoryordersalestatus__status__in=status_noworks_all).distinct().count()),str(model_order_buy.filter(filter_group).filter(author=user,create_date__range=(date_start,date_stop)).filter(hystoryorderbuystatus__status__in=status_works_all).exclude(hystoryorderbuystatus__status__in=status_noworks_all).distinct().count())))
                    content_item["orders_growth_rn"] = "/".join((str(int(content_item["orders_growth"].split("/")[0]) - int(content_item["orders_growth_agent"].split("/")[0])), str(int(content_item["orders_growth"].split("/")[1]) - int(content_item["orders_growth_agent"].split("/")[1]))))
                    content_item["orders_retired"] = "/".join((str(model_order_sale.filter(filter_group).filter(performer=user).filter(hystoryordersalestatus__date__range=(date_start,date_stop),hystoryordersalestatus__status=status_retired).distinct().count()),str(model_order_buy.filter(filter_group).filter(performer=user).filter(hystoryorderbuystatus__date__range=(date_start,date_stop),hystoryorderbuystatus__status=status_retired).distinct().count())))
                    # договоры: предложение/спрос
                    content_item["orders_contract_all"] = "/".join((str(model_order_sale.filter(filter_group).filter(performer=user,contract_date__lte=date_stop,contract_type__in=(contract_all)).exclude(hystoryordersalestatus__status__in=status_noworks).distinct().count()),str(model_order_buy.filter(filter_group).filter(performer=user,contract_date__lte=date_stop,contract_type__in=(contract_all)).exclude(hystoryorderbuystatus__status__in=status_noworks).distinct().count())))
                    content_item["orders_contract_growth"] = "/".join((str(model_order_sale.filter(filter_group).filter(performer=user,contract_date__range=(date_start,date_stop),contract_type__in=(contract_all)).exclude(hystoryordersalestatus__status__in=status_noworks_all).distinct().count()),str(model_order_buy.filter(filter_group).filter(performer=user,contract_date__range=(date_start,date_stop),contract_type__in=(contract_all)).exclude(hystoryorderbuystatus__status__in=status_noworks_all).distinct().count())))
                    # актов показов за период
                    content_item["shows_acts_all"] = str(models.HystoryShow.objects.filter(offer__order_buy__object_category__in=object_category,offer__order_buy__performer=user,date__range=(date_start,date_stop)).exclude(result=status_show_work).count()+models.HystoryShow.objects.filter(offer__order_sale__object_category__in=object_category,offer__order_sale__performer=user,date__range=(date_start,date_stop)).exclude(result=status_show_work).distinct().count())
                    # сделок за период
                    content_item["transactions"] = str(model_order_sale.filter(filter_group).filter(performer=user).filter(offer__hystoryservice__date__range=(date_start,date_stop),offer__hystoryservice__operation=operation_registr,offer__hystoryservice__result_operation=operation_complet).distinct().count()+model_order_buy.filter(filter_group).filter(performer=user).filter(offer__hystoryservice__date__range=(date_start,date_stop),offer__hystoryservice__operation=operation_registr,offer__hystoryservice__result_operation=operation_complet).distinct().count())
                    data["messages"]["content"].append(content_item.copy())
                ...
                content_item["orders_retired"] = "/".join((str(round(model_order_sale.filter(filter_brigade).filter(filter_group).filter(hystoryordersalestatus__date__range=(date_start,date_stop),hystoryordersalestatus__status=status_retired).distinct().count()/agent_count,1)),str(round(model_order_buy.filter(filter_brigade).filter(filter_group).filter(hystoryorderbuystatus__date__range=(date_start,date_stop),hystoryorderbuystatus__status=status_retired).distinct().count()/agent_count,1))))
                content_item["orders_growth_rn"] = ""
                content_item["orders_growth_agent"] = ""
                # договоры: предложение/спрос
                content_item["orders_contract_all"] = "/".join((str(round(model_order_sale.filter(filter_brigade).filter(filter_group).filter(contract_date__lte=date_stop,contract_type__in=(contract_all)).exclude(hystoryordersalestatus__status__in=status_noworks).distinct().count()/agent_count,1)),str(round(model_order_buy.filter(filter_brigade).filter(filter_group).filter(contract_date__lte=date_stop,contract_type__in=(contract_all)).exclude(hystoryorderbuystatus__status__in=status_noworks).distinct().count()/agent_count,1))))
                content_item["orders_contract_growth"] = "/".join((str(round(model_order_sale.filter(filter_brigade).filter(filter_group).filter(contract_date__range=(date_start,date_stop),contract_type__in=(contract_all)).exclude(hystoryordersalestatus__status__in=status_noworks_all).distinct().count()/agent_count,1)),str(round(model_order_buy.filter(filter_brigade).filter(filter_group).filter(contract_date__range=(date_start,date_stop),contract_type__in=(contract_all)).exclude(hystoryorderbuystatus__status__in=status_noworks_all).distinct().count()/agent_count,1))))
                # актов показов за период
                content_item["shows_acts_all"] = str(round(models.HystoryShow.objects.filter(filter_brigade_show).filter(offer__order_buy__object_category__in=object_category,date__range=(date_start,date_stop)).exclude(result=status_show_work).distinct().count()/agent_count,1))
                # сделок за период
                content_item["transactions"] = str(round((model_order_sale.filter(filter_brigade).filter(filter_group).filter(offer__hystoryservice__date__range=(date_start,date_stop),offer__hystoryservice__operation=operation_registr,offer__hystoryservice__result_operation=operation_complet).distinct().count()+model_order_buy.filter(filter_brigade).filter(filter_group).filter(offer__hystoryservice__date__range=(date_start,date_stop),offer__hystoryservice__operation=operation_registr,offer__hystoryservice__result_operation=operation_complet).distinct().count())/agent_count,1))
                data["messages"]["content"].append(content_item.copy())
